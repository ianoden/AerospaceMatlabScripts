...
t06 = 0.06; % max thickness of NACA 0006 
t12 = 0.12; % max thickness of NACA 0012
t18 = 0.18; % max thickness of NACA 0018

sz = 100;

vInf = 100;   %velocity infinity test
AOA = linspace(-24,32,sz-1);

c = 1; % chord length test
x = linspace(0,1,sz);  % x location as percent of chord length
...
for i = 1:sz
    yt06(i) = (t06.*c./0.2).*(0.2969.*(x(i)./c).^(0.5)-0.1260.*(x(i)./c).^1-0.3516.* ...
        (x(i)./c).^2+0.2843.*(x(i)./c).^3-0.1036.*(x(i)./c).^4);

    yt12(i) = (t12.*c./0.2).*(0.2969.*(x(i)./c).^(0.5)-0.1260.*(x(i)./c).^1-0.3516.* ...
        (x(i)./c).^2+0.2843.*(x(i)./c).^3-0.1036.*(x(i)./c).^4);

    yt18(i) = (t18.*c./0.2).*(0.2969.*(x(i)./c).^(0.5)-0.1260.*(x(i)./c).^1-0.3516.* ...
        (x(i)./c).^2+0.2843.*(x(i)./c).^3-0.1036.*(x(i)./c).^4);
    %symmetric airfoil so only need thickness from mean camber line
    %mean camber line = centerline
end


for j = 1:99
    CL06(j) = Vortex_Panel(x,yt06,vInf,AOA(j));

    CL12(j) = Vortex_Panel(x,yt12,vInf,AOA(j));

    CL18(j) = Vortex_Panel(x,yt18,vInf,AOA(j));
end
